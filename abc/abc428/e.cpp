...
namespace algologic {  // https://algo-logic.info/tree-dp/
...
struct Rerooting {
    ...
    struct DP {  // DP の型
        long long dp;
        int v;
        DP(long long dp_, int v_) : dp(dp_), v(v_) {}
    };
    const DP identity = DP(-1, -1);  // 単位元(末端の値は add_root(identity) になるので注意)
    function<DP(DP, DP)> merge = [](DP dp_cum, DP d) -> DP {
        DP ret(-1, -1);
        ret.dp = max(dp_cum.dp, d.dp);
        if (dp_cum.dp == d.dp) {
            ret.v = max(dp_cum.v, d.v);
        } else {
            ret.v = dp_cum.dp > d.dp ? dp_cum.v : d.v;
        }
        return ret;
    };
    function<DP(DP)> add_root = [](DP d) -> DP { return DP(d.dp + 1, d.v); };
    /* end 問題ごとに書き換え */
    // グラフの定義
    struct Edge {
        int to;
...
int main() {
    ...
    reroot.build();
    for (int i = 0; i < n; i++) std::cout << reroot.ans[i].v + 1 << '\n';
    std::cout << std::flush;
}